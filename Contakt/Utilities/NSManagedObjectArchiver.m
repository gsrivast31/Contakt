...
- (id)copyUsingContext:(NSManagedObjectContext *)pContext insert:(BOOL)pInsert {
    ...
    NSMutableArray* keys = [NSMutableArray array];
    for (NSString* key in self.entity.attributesByName.keyEnumerator) {
        [keys addObject:key];
    }
    [copy setValuesForKeysWithDictionary:[self dictionaryWithValuesForKeys:keys]];
    return copy;
}
