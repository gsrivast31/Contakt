    ...
    if ([self.textFields count] == 0){
        for(UIView *subView in view.subviews){
            if ([subView isKindOfClass:[CKTextField class]]){
                CKTextField *textField = (CKTextField*)subView;
                textField.tag = index;
...
- (void) nextButtonIsClicked:(id)sender{
    ...
    CKTextField *textField =  [self.textFields objectAtIndex:++tagIndex];
...
- (void) keyboardDidShow:(NSNotification *) notification{
    ...
    if (![_textField isKindOfClass:[CKTextField class]]) return;
...
- (void)textFieldDidBeginEditing:(NSNotification *) notification{
    FUITextField *textField = (FUITextField*)[notification object];
